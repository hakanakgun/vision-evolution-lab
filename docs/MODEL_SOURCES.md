...
- Export workflow: `.github/workflows/yolov1-feasibility.yml` calls `scripts/yolov1_feasibility.py`, exports fixed-shape ONNX, dynamically INT8-quantizes it, verifies the canonical dog/bicycle/car result, then opens and runs the candidate through `onnxruntime-web@1.30.0` WASM before publishing.
- Published browser asset: GitHub Release `yolov1-browser-int8-122bf7462747`, file `yolov1-voc20-int8.onnx`, 541,358,513 bytes (~516.3 MiB), SHA-256 `122bf7462747d0cf140525ed6c1d90424d64cc10b8d96cf17905343ce0306d49`.
...
- Hugging Face publication: the workflow also supports publishing the verified INT8 artifact to a user-owned Hub repository when a GitHub Actions `HF_TOKEN` secret is configured. The current run had no such secret, so the immutable browser source is the checksum-pinned GitHub Release; the original source checkpoint remains pinned on Hugging Face.
