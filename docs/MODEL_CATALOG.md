...
| 2021-06 | YOLOS-tiny | Runnable | pure/vanilla Vision Transformer used directly for object detection | pinned `Xenova/yolos-tiny` q4 conversion of Apache-2.0 `hustvl/yolos-tiny`; HUST code MIT; conversion license not independently declared | Time Machine only; Transformers.js/WASM q4; no Model Race, Live Camera, benchmark, or Inside the Model until broader validation |
...
YOLOS provides a materially different 2021 branch: a vanilla Vision Transformer adapted to COCO object detection without a convolutional detector backbone. Vision Evolution Lab uses a pinned q4 Transformers.js conversion and WASM path. It is Time Machine-only until mobile/WebKit and multi-image behavior are verified.
