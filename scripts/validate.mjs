...
check(files.app.includes("setStatus('Running '+activeModel().title+' on the current Time Machine image…','loading')")&&files.app.includes('void runUploaded();'),'selecting a runnable Time Machine model with an existing image must auto-run that image');
check(!files.app.includes("selected. Run the current image when ready."),'Time Machine selection must not require a second manual Run action when an image is already selected');
...
for(const [year,month,title] of [[2020,5,'DETR · ResNet-50'],[2021,6,'YOLOS-tiny'],[2021,7,'YOLOX-Nano'],[2023,4,'RT-DETR R18'],[2024,6,'LW-DETR-tiny'],[2024,7,'RT-DETRv2 R18'],[2024,10,'D-FINE-N']])check(metadataRegistry.timeline.some(entry=>entry.year===year&&entry.month===month&&entry.title===title),'timeline month missing or incorrect: '+title);
...
const yolos=metadataRegistry.yolos,yolosEntry=metadataRegistry.timeline.find(item=>item.model==='yolos');
