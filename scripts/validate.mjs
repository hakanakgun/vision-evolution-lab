...
check(JSON.stringify(decoded.detections[0].box)==='[0.3,0.3,0.7,0.7]','LW-DETR cxcywh-to-normalized-yxyx conversion changed');
...
check(files.historicalDetectors.includes("runtimes.register('lwdetr'")&&files.historicalDetectors.includes("runtimes.register('dfine'")&&files.historicalDetectors.includes("model.modelId,{device:'wasm',dtype:'fp32',revision:model.revision"),'D-FINE-N adapter must use Transformers.js on the pinned WASM fp32 conversion');
