## What changed

<!-- Keep this focused. -->

## Why

<!-- Explain the problem, research/learning goal, or bug being addressed. -->

## Validation

- [ ] `node --check app.js` passes when JavaScript changed.
- [ ] No duplicate HTML IDs were introduced.
- [ ] Relevant image inference flow was tested.
- [ ] Benchmark changes were checked with the 20-run warm benchmark.
- [ ] Desktop and narrow/mobile layout were checked when UI changed.
- [ ] No unrelated changes are included.

## Models / dependencies / provenance

- [ ] No model, weights, dataset, runtime, or dependency provenance changed.
- [ ] Or: `MODEL_SOURCES.md` / `THIRD_PARTY_LICENSES.md` was updated with the exact source and license information.

## Privacy

- [ ] This change does not upload user images/camera frames or introduce telemetry.
- [ ] Or: any new remote-data behavior is explicitly described and justified above.

## Screenshots / notes

<!-- Avoid attaching private or sensitive source images. -->
