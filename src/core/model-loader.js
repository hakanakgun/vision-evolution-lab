(() => {
  ...
  async function readResponse(response,onProgress,label,expectedBytes=0){
    ...
    if(!response.body||!response.body.getReader){const buffer=await response.arrayBuffer();onProgress?.({label,loaded:buffer.byteLength,total:total||buffer.byteLength,percent:total?Math.min(100,buffer.byteLength/total*100):100});return buffer}
    const reader=response.body.getReader(),chunks=[];let loaded=0;
    while(true){const {done,value}=await reader.read();if(done)break;chunks.push(value);loaded+=value.byteLength;onProgress?.({label,loaded,total,percent:total?Math.min(100,loaded/total*100):null})}
    ...
    onProgress?.({label,loaded,total:total||loaded,percent:total?Math.min(100,loaded/total*100):100});return merged.buffer;
  }
  ...
  async function load(model,{onState,onProgress,onTrace}={}){
    ...
    for(const source of model.sources||[])for(let attempt=0;attempt<2;attempt++){const t=performance.now();try{
      ...
      const type=response.headers.get('content-type')||'application/octet-stream',buffer=await readResponse(response,onProgress,source.label,model.bytes);trace('arraybuffer-obtained',{source:source.label,origin:'fetch',bytes:buffer.byteLength});
