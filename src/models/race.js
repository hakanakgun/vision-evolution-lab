...
${tail||'—'}`;}
  ...
  async function runRT(source,canvas,{forceBackend='',allowFallback=true,confidence}={}){
  ...
  function createRtResearchAdapter(modelKey){
    ...
    async function run(source,canvas,{forceBackend='',allowFallback=true,confidence}={}){
      ...
      prepareDisplay(source,canvas);const drawStarted=performance.now(),visible=api.drawDetections(canvas,detections,confidence),postMs=performance.now()-drawStarted;
  ...
  const SANITY_SUITE_URL='assets/benchmark/sanity-suite.json';
  async function loadSanityImage(url){return await new Promise((resolve,reject)=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>reject(new Error('Sanity image could not be decoded: '+url));image.src=url})}
  function sanityTargets(data,key){const all=(data.instances||[]).map(item=>({label:item.label,box:[item.bbox[1]/data.height,item.bbox[0]/data.width,(item.bbox[1]+item.bbox[3])/data.height,(item.bbox[0]+item.bbox[2])/data.width]}));return key==='tinyyolo'?all.filter(item=>registry.labels.vocCanonical.includes(item.label)):all}
  function appendSanityRow(body,label,result,error=''){
    const row=document.createElement('div');row.className='race-benchmark-row accuracy-row';appendText(row,'span',label);
    const values=error?['failed','—','—',error]:[percent(result.precision),percent(result.recall),percent(result.f1),result.truePositives+' / '+result.falsePositives+' / '+result.falseNegatives];
    for(const value of values)appendText(row,'b',value);body.appendChild(row);
  }
  function percent(value){return Number.isFinite(value)?(value*100).toFixed(1)+'%':'—'}
  async function runAccuracySanity(){
    if(state.running||state.benchmarking||state.sanityRunning)return;
    const button=$('race-sanity-run'),body=$('race-sanity-body'),status=$('race-sanity-status'),hidden=document.createElement('canvas');
    state.sanityRunning=true;button.disabled=true;body.replaceChildren();
    for(const id of ['race-image-file','race-use-current','race-use-coco-sample','race-run','race-benchmark'])$(id).disabled=true;
    let failures=0;
    try{
      const response=await fetch(SANITY_SUITE_URL,{cache:'no-store'});if(!response.ok)throw new Error('Sanity-suite manifest HTTP '+response.status);
      const manifest=await response.json(),samples=Array.isArray(manifest.samples)?manifest.samples:[],confidence=Number(manifest.matching?.confidence??.4),iouThreshold=Number(manifest.matching?.iou??.5);
      if(!samples.length)throw new Error('Sanity-suite manifest has no samples.');
      const adapters=runtimeRegistry.list({capability:'race',group:'general-object'});
      await releaseRaceRuntimes();
      for(let modelIndex=0;modelIndex<adapters.length;modelIndex++){
        const adapter=adapters[modelIndex],totals={truePositives:0,falsePositives:0,falseNegatives:0};
        try{
          for(let sampleIndex=0;sampleIndex<samples.length;sampleIndex++){
            const sample=samples[sampleIndex];status.textContent='Sanity '+(modelIndex+1)+'/'+adapters.length+' · '+adapter.model.title+' · image '+(sampleIndex+1)+'/'+samples.length+'…';
            const [image,annotationResponse]=await Promise.all([loadSanityImage(sample.image),fetch(sample.annotations,{cache:'no-store'})]);
            if(!annotationResponse.ok)throw new Error('Annotation HTTP '+annotationResponse.status+' for '+sample.id);
            const annotations=await annotationResponse.json(),run=await adapter.run(image,hidden,{confidence}),targets=sanityTargets(annotations,adapter.key);
            const measured=window.VisionDetectionMetrics.evaluate(run.detections,targets,{confidence,iouThreshold});
            totals.truePositives+=measured.truePositives;totals.falsePositives+=measured.falsePositives;totals.falseNegatives+=measured.falseNegatives;
            hidden.width=1;hidden.height=1;image.src='';
          }
          const pDen=totals.truePositives+totals.falsePositives,rDen=totals.truePositives+totals.falseNegatives;
          totals.precision=pDen?totals.truePositives/pDen:0;totals.recall=rDen?totals.truePositives/rDen:0;totals.f1=totals.precision+totals.recall?2*totals.precision*totals.recall/(totals.precision+totals.recall):0;
          appendSanityRow(body,adapter.model.title,totals);
        }catch(error){
          failures++;console.error(error);appendSanityRow(body,adapter.model.title,null,String(error.message||error).slice(0,80));
        }finally{await adapter.release?.()}
      }
      status.textContent='Fixed '+samples.length+'-image COCO 2017 val sanity suite complete · confidence '+confidence.toFixed(2)+' · same class · IoU ≥ '+iouThreshold.toFixed(2)+'. '+(failures?failures+' model(s) failed; inspect the row above. ':'')+'Aggregated precision / recall / F1 is browser implementation regression evidence, not COCO AP and not a model ranking.';
    }catch(error){console.error(error);status.textContent='Accuracy sanity failed: '+String(error.message||error)}
    finally{
      await releaseRaceRuntimes();hidden.width=1;hidden.height=1;state.sanityRunning=false;button.disabled=false;
      $('race-image-file').disabled=false;$('race-use-current').disabled=false;$('race-use-coco-sample').disabled=false;$('race-run').disabled=!state.image;$('race-benchmark').disabled=!state.image;
    }
  }
  ...
  async function runRace(){
    if(!state.image||state.running||state.sanityRunning)return;
