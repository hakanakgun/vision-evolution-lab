  (() => {
    ...
    const INSIDE_SPECS=Object.freeze({
      tinyyolo:Object.freeze({title:'Tiny YOLOv2',input:'416×416',resize:'direct stretch to 416×416',tensor:'float32 · NCHW',channels:'RGB',normalization:'raw 0–255 float input',step2:['Direct resize','The source is resized directly to 416×416, matching the exact-export runtime convention rather than letterboxed.'],step3:['RGB tensor','Canvas RGB byte values are packed as float32 NCHW. Upstream preprocessing documentation is incomplete, so this is explicitly recorded as a validation-sensitive contract.'],step4:['YOLOv2 grid head','A 13×13 grid predicts 5 anchors per cell, each with box/objectness plus 20 Pascal VOC class logits.'],caption:'Tiny YOLOv2 416×416 direct-resize preview',intermediate:['Tiny YOLOv2 intermediate tensors not exported','The current export exposes the final 125×13×13 detection grid, not selected backbone activations.','No simulated feature maps are shown; selected real intermediate outputs would require an inspectable export.']}),
    ...
    async function runBenchmark(){
      ...
      try{
        ...
        $('benchmark-note').textContent=benchmarkModel==='rtdetr'?'p50, p90, min–max and CV from 20 warm Transformers.js pipeline calls; pipeline/model load excluded.':'p50, p90, min–max and CV from 20 sequential warm runs; model transfer and session init excluded.';
    ...
    RuntimeRegistry.register('ssd',{
      run:(source,canvas)=>inferSource(source,canvas,{updateMain:false}),
