...
- Temporary iOS diagnostic: `?diag=1` keeps the original lifecycle probe; `?diag=2` adds an on-screen persistent Diagnostic Black Box with heartbeat age, event-loop gap, lifecycle/navigation state, runtime residency, RT staging-canvas size, available JS-memory telemetry, and the last 40 diagnostic events so an unexpected WebKit reload can be read directly after recovery. Normal Model Race behavior is unchanged. Remove this instrumentation after the device test.
