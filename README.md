...
- Temporary iOS diagnostic: `?diag=1` keeps the basic lifecycle probe; `?diag=2` adds a model-agnostic diagnostic planner generated from the Model Race runtime adapters. Any subset of registered race models can run in race order, with the final selected runtime intentionally left resident. Diagnostic adapters may expose backend-isolation choices; YOLOX now offers Auto, forced WebGPU, and forced WASM, while RT-DETR offers Auto, forced WebGPU fp16, and forced WASM q8. Forcing either backend disables that model's normal WebGPU → WASM fallback so the probe isolates one execution path. The diagnostic also retains the 0/250/1000/3000 ms inter-model settle probe and now records start/complete breadcrumbs for every measured run of the final selected runtime. The persistent Black Box records a persistent diagnostic attempt number, selected plan, backend probes, actual backends, settle value, heartbeat age, lifecycle/navigation state, runtime residency, work-canvas sizes, available JS-memory telemetry, and the last 40 diagnostic events; a small Copy Black Box control copies the visible diagnostic text with a clipboard fallback for iOS/WebKit. A diagnostic-only reclamation matrix locks all four models to WASM, keeps inter-model settle at 0 ms, and runs 10 attempts each for R1: final RT resident + 0 ms inter-attempt delay, R2: final RT disposed + 0 ms, R3: final RT disposed + 3000 ms, and R4: final RT disposed + 10000 ms. Matrix progress and per-case outcomes are persisted. An abrupt reload during an active attempt is recorded with the exact case/attempt and durable last stage, then the next case is queued; after reload the user reselects the same source image and presses Resume reclamation matrix. Diagnostic stage snapshots are written durably on every stage transition so the previous-stage report is not limited to the 1-second heartbeat. Normal Model Race behavior is unchanged; this instrumentation is temporary and does not claim the iOS reload is fixed.
