...
- Model Race remains a separate three-generation comparison: SSD-MobileNetV1 INT8 (2017), YOLOX-Nano (2021), and RT-DETR R18 (2023, Transformers.js/ONNX), with a 20-run warm benchmark. RT-DETR keeps the same pinned checkpoint revision but moves from Transformers.js 3.8.1 to 4.3.0/native WebGPU EP to address the legacy `AveragePool ceil()` runtime failure; WASM q8 remains fallback.
...
Completed foundations now include the browser benchmark layer, three-model race, sourced evolution atlas, preprocessing inspector, and a Time Machine active-model runner. Immediate validation remains real-device RT-DETR inference after the native WebGPU runtime upgrade; next candidates include Failure Gallery, Classical CV vs AI, Efficiency Lab, Resolution Microscope, and deeper feature inspection.
