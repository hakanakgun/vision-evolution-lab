...
- **Architecture Explorer** compares two runnable Time Machine generations side by side using the same declared family, preprocessing, detector-path, output, runtime/capability, and intermediate-evidence contracts that already drive the app. It does not run inference or reconstruct hidden layers.
...
- **YOLOv1** now runs in Time Machine through a pinned 448×448 VOC20 ONNX/WASM export. The dynamic-INT8 file is still ~516.3 MiB, so an uncached run shows a large-download warning before transfer and exposes **Cancel download** while bytes are moving. It remains outside Model Race and Live Camera; physical mobile behavior is unverified.
- **Faster R-CNN R50-FPN INT8** turns the 2015 two-stage RPN milestone into a runnable Time Machine reference on ONNX Runtime Web/WASM. It remains outside Model Race, Live Camera, and individual benchmarks while broader portability is validated. Inside the Model shows the verified two-stage contract and real final detections, while stating that RPN proposal tensors are internal to this export and are not fabricated.
- **YOLOS-tiny** adds a pure Vision Transformer detector through a pinned Transformers.js q4/WASM conversion. It remains Time Machine-only for execution scope pending broader browser/device validation; Inside the Model explains its patch/detection-token transformer pipeline without inventing token or attention tensors.
- **D-FINE-N** runs in Time Machine and Live Camera through a pinned Transformers.js ONNX conversion on WASM fp32. It remains excluded from Model Race and individual benchmarks; Inside the Model exposes the verified pipeline contract but no internal query/regression tensors. Mobile performance and iOS/WebKit Live Camera behavior have not yet been benchmarked; broader device/sample validation remains pending.
- **LW-DETR-tiny** runs in Time Machine and Live Camera through a reproducibly exported, checksum-pinned ONNX checkpoint on ONNX Runtime Web/WASM fp32. It remains excluded from Model Race and individual benchmarks. Inside the Model documents the real `logits`/`pred_boxes` output contract without retaining or fabricating query visualizations. One user-reported iOS 18.7 / Brave-WebKit Benchmark ×20 completed on 2026-09-24 at about 612 ms p50 inference; sustained Live Camera stability and broad device performance remain unverified. See the iOS diagnostics.
...
Selecting a runnable model in Time Machine stays in Time Machine and, when a source image is already present, immediately reruns that image with the new model. Inside the Model follows the same active model. Live Camera keeps an independent model selection and independent confidence threshold, so changing either Live setting does not silently change Time Machine.
...
| 2016 | YOLOv1 | Pascal VOC 20 | Time Machine · derived dynamic-INT8 ONNX · WASM · ~516.3 MiB |
...
Model Race keeps the existing single-image COCO smoke check and adds a separate fixed four-image COCO 2017 validation sanity suite. The suite uses confidence 0.40, same-class matching and IoU ≥ 0.50, aggregates TP/FP/FN across the four images, and derives precision/recall/F1. Tiny YOLOv2 is scored only on annotations whose classes overlap Pascal VOC. The suite is intended to catch browser preprocessing/postprocessing regressions; it is not COCO AP, does not represent the full validation distribution, and is not a general model ranking. See [Benchmark samples](docs/BENCHMARK_SAMPLES.md) for attribution and scope.
...
7. Architecture Explorer — shipped in v0.21.0
...
The iOS/WebKit reload mitigation has passed the current physical-device regression test. Further diagnostic work is only needed if the issue recurs. Efficiency Lab, Resolution Microscope, and Architecture Explorer are now executable. Remaining validation work is primarily physical-device coverage and deliberately deferred Failure Gallery work.
