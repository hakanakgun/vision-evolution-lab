...
Selecting a model in Time Machine stays in Time Machine. Inside the Model follows the same active model. Live Camera keeps an independent model selection and independent confidence threshold, so changing either Live setting does not silently change Time Machine.
