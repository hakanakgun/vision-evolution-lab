...
- Temporary iOS diagnostic: `?diag=1` keeps the basic lifecycle probe; `?diag=2` adds a model-agnostic diagnostic planner generated from the Model Race runtime adapters. Any subset of current or future registered race models can run in race order, with the final selected runtime intentionally left resident. The on-screen persistent Black Box records the selected plan, heartbeat age, event-loop gap, lifecycle/navigation state, per-model runtime residency, registered work-canvas sizes, available JS-memory telemetry, and the last 40 diagnostic events. Normal Model Race behavior is unchanged. Remove this instrumentation after the device test.
