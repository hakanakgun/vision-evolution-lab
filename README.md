...
- Inside the Model now follows the active Time Machine model. It shows each model's native preprocessing contract and prepared-input preview; YOLOX additionally exposes real pre-NMS detection-head objectness maps at strides 8/16/32, while SSD/RT-DETR explicitly state that deeper intermediate tensors are not yet exported.
- Live Camera mode with sequential inference and rolling latency measurements.
- Model Race remains a separate three-generation comparison: SSD-MobileNetV1 INT8 (2017), YOLOX-Nano (2021), and RT-DETR R18 (2023, Transformers.js/ONNX), with a 20-run warm benchmark and three-model pairwise detection-overlap summary. The UI confidence threshold is applied to retained outputs so slider changes can redraw YOLOX/RT-DETR results without rerunning inference; run/benchmark operations lock source, model and confidence. RT-DETR keeps the same pinned checkpoint revision on Transformers.js 4.3.0/native WebGPU EP with WASM q8 fallback.
